...
// SPIバスの設定
    ...
    { // タッチスクリーン制御の設定を行います。（必要なければ削除）
      ...
      cfg.x_min      = 400;    // タッチスクリーンから得られる最小のX値(生の値)
      cfg.x_max      = 3800;  // タッチスクリーンから得られる最大のX値(生の値)
      cfg.y_min      = 500;    // タッチスクリーンから得られる最小のY値(生の値)
      cfg.y_max      = 3500;  // タッチスクリーンから得られる最大のY値(生の値)
      cfg.pin_int    = 255;   // INTが接続されているピン番号 // was 22, 4
